...
'
'
' This table is meant to be a starting point. It implements VPW physics and Fleep sounds. More detailed 
' scripts and resources can be found within the main VPW Example table. 
'
' This table includes a playfield image that shows a standard Stern lower third layout. 
'
' Copy and paste the walls, primitives, ramps, and targets on the left side of the table as needed.
'
' *NOTE!* You must include all new rubber posts in the dPosts collection, and all new rubber sleeves in the
' dSleeves collection. Also, include the other walls in their respective Fleep collections (Rubbers, Metals, Walls, etc) 
' if you want their mechanical sound effects to work.
'
...
' CHANGE LOG
'1.6.3 apophis - Initial version that aligns with the Example Table VPW 1.6.3
'1.6.4 apophis - Fixed default EOSReturn setting. Converted some Fleep sounds to mono.
'Release 1.7
'1.7.1 apophis - Fixed dSleeves collection name
...
Const cGameName = "Haunted Mansion"		 'The unique alphanumeric name for this table
...
Dim HMBall1, HMBall2, HMBall3, HMBall4, HMBall5, gBOT

Sub Table1_Init
	Dim i
	
	'Ball initializations need for physical trough
	Set HMBall1 = swTrough1.CreateSizedballWithMass(Ballsize / 2,Ballmass)
	Set HMBall2 = swTrough2.CreateSizedballWithMass(Ballsize / 2,Ballmass)
	Set HMBall3 = swTrough3.CreateSizedballWithMass(Ballsize / 2,Ballmass)
	Set HMBall4 = swTrough4.CreateSizedballWithMass(Ballsize / 2,Ballmass)
	Set HMBall5 = swTrough5.CreateSizedballWithMass(Ballsize / 2,Ballmass)
	
	'*** Use gBOT in the script wherever BOT is normally used. Then there is no need for GetBalls calls ***
	gBOT = Array(HMBall1, HMBall2, HMBall3, HMBall4, HMBall5)
...
Sub ResetDropTargets
	Dim DT
	For each DT in DropTargets: DT.isDropped = False: Next
	RandomSoundDropTargetReset SFXPos
End Sub

Sub DropTargets_hit(x)
	SoundDropTargetDrop DropTargets(x)
End Sub
